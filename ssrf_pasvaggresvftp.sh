...
function run_task {
    local n_ip=$1 port=$2 fds fds_PID fd_in fd_out ip pid varname_pid handler req_line=
    ...
    #simultaneous connections funcitonality depends on nc being compiled with
...
function ftp_server {
    ...
    debug_print 2 "$connection_info"
    cmd_in=_
    while [[ -n $cmd_in && ! $cmd_in =~ RETR|LIST|QUIT ]];do
        read elapsed_time cmd_in < <(read_data)
        debug_print 2 "Received: $cmd_in in ${elapsed_time}ms"
        case $cmd_in in
            USER*)
